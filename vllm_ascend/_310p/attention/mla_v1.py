...
class AscendMLAImpl310(MLAAttentionImpl):
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        ...
        if not isinstance(self.kv_b_proj.quant_method, UnquantizedLinearMethod):
            logger.warning_once(
                "310P MLA currently only supports unquantized kv_b_proj. "
                "V up-projection may not work correctly.",
            )
            return
    ...
    def _forward_prefill(
        ...
        real_tokens = int(prefill_meta["seq_lens"][: attn_metadata.num_prefills].sum().item())
        seq_len = prefill_meta["seq_lens"][: attn_metadata.num_prefills]
