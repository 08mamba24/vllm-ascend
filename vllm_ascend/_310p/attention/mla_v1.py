...
class AscendMLAImpl310(MLAAttentionImpl):
    ...
    def _mla_preprocess_decode(
        ...
        if self.qk_rope_head_dim is None:
            raise RuntimeError("qk_rope_head_dim must be set for MLA RoPE")
        cos, sin = self._prepare_rope_cos_sin(
            decode_meta["cos"],
            decode_meta["sin"],
            num_tokens=bsz,
        ...
        if self.kv_lora_rank is not None and self.kv_a_layernorm is not None and self.fused_qkv_a_proj is not None:
            ...
            if self.kv_a_proj_with_mqa is None:
                raise RuntimeError("kv_a_proj_with_mqa is required when fused_qkv_a_proj is not used")
    ...
    def _store_to_cache_310(
        ...
        if not hasattr(self, "_debug_dumped_cache_write"):
            self._debug_dumped_cache_write = True
            try:
                slots_head = slots[:10].tolist()
                block_pairs = []
                for slot in slots_head:
                    block_pairs.append((int(slot) // block_size, int(slot) % block_size))
                logger.info(
                    "[310P MLA][DEBUG] cache write slots head: %s",
                    slots_head,
                )
                logger.info(
                    "[310P MLA][DEBUG] cache write block/offset head: %s",
                    block_pairs,
                )
                logger.info(
                    "[310P MLA][DEBUG] cache write kv_c shape=%s dtype=%s",
                    tuple(kv_c.shape),
                    kv_c.dtype,
                )
                logger.info(
                    "[310P MLA][DEBUG] cache write k_pe shape=%s dtype=%s",
                    tuple(k_pe.shape),
                    k_pe.dtype,
                )
                if kv_c.dim() == 2:
                    kv_c_sample = kv_c[:1, :4].detach().cpu()
                elif kv_c.dim() >= 3:
                    kv_c_sample = kv_c[:1, :2, :4].detach().cpu()
                else:
                    kv_c_sample = kv_c.flatten()[:8].detach().cpu()

                if k_pe.dim() == 2:
                    k_pe_sample = k_pe[:1, :4].detach().cpu()
                elif k_pe.dim() >= 3:
                    k_pe_sample = k_pe[:1, :2, :4].detach().cpu()
                else:
                    k_pe_sample = k_pe.flatten()[:8].detach().cpu()
                logger.info(
                    "[310P MLA][DEBUG] cache write kv_c sample: %s",
                    kv_c_sample.tolist(),
                )
                logger.info(
                    "[310P MLA][DEBUG] cache write k_pe sample: %s",
                    k_pe_sample.tolist(),
                )
            except Exception as e:
                logger.warning("[310P MLA][DEBUG] cache write debug dump failed: %s", e)

        num_blocks = kv_c_cache.shape[0]
        valid_mask = (slots // block_size) < num_blocks
        valid_slots = slots[valid_mask].long()

        if valid_slots.numel() > 0:
            kv_c_cache.view(-1, *kv_c_cache.shape[2:]).index_copy_(
                0, valid_slots, kv_c_expanded[valid_mask].to(kv_c_cache.dtype)
            )
            k_pe_cache.view(-1, *k_pe_cache.shape[2:]).index_copy_(
                0, valid_slots, k_pe_expanded[valid_mask].to(k_pe_cache.dtype)
            )

    def _prepare_rope_cos_sin(
        self,
        cos: torch.Tensor,
        sin: torch.Tensor,
        num_tokens: int,
        ...
        """Normalize cached cos/sin into [T, C] before RoPE application.

        MLA decode may use static cache buffers while prefill may use sliced views,
        so this method accepts [T, C], [T, 1, C], and [T, 1, 1, C] inputs and
        ensures token-aligned 2D tensors.
        """
        if cos is None or sin is None:
            raise RuntimeError(f"RoPE cache is not initialized for {source} path")

        if cos.dim() == 4:
            if cos.shape[1] != 1 or cos.shape[2] != 1 or sin.shape[1] != 1 or sin.shape[2] != 1:
                raise RuntimeError(
                    f"Unexpected RoPE cache rank-4 shape in {source}: cos={tuple(cos.shape)}, sin={tuple(sin.shape)}"
                )
            ...
            if cos.shape[1] != 1 or sin.shape[1] != 1:
                raise RuntimeError(
                    f"Unexpected RoPE cache rank-3 shape in {source}: cos={tuple(cos.shape)}, sin={tuple(sin.shape)}"
                )
            cos = cos.squeeze(1)
            sin = sin.squeeze(1)
        ...
        if cos.shape[0] < num_tokens:
            raise RuntimeError(
                f"Insufficient RoPE cache rows in {source}: got {cos.shape[0]}, expected at least {num_tokens}"
            )
        if cos.shape[0] > num_tokens:
            cos = cos[:num_tokens]
            sin = sin[:num_tokens]

        if cos.shape[-1] not in {rope_dim // 2, rope_dim, rope_dim * 2}:
            logger.warning_once(
                "[310P MLA] Unexpected RoPE cache width in %s: cos=%s sin=%s rope_dim=%s",
                source,
                tuple(cos.shape),
                tuple(sin.shape),
                rope_dim,
    ...
    def _rope_single(self, x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
        if x.dim() != 3:
            raise RuntimeError(f"_rope_single expects x as [T, H, D], got shape={tuple(x.shape)}")
        ...
        rope_dim_half = D // 2
        cache_dim = cos.shape[-1]

        # GLM uses non-neox (interleaved) RoPE.
        # We normalize cache to [c0, c0, c1, c1, ...] so that it can be applied
        # directly with rotate_every_two style pairing.
        if cache_dim == rope_dim_half:
            # [T, D/2] -> [T, D]
            cos = cos.repeat_interleave(2, dim=-1)
            sin = sin.repeat_interleave(2, dim=-1)
        elif cache_dim == D:
            # [T, D] can be either already interleaved or duplicated halves.
            # If duplicated ([a0..aN, a0..aN]), convert via first half.
            if torch.equal(cos[..., :rope_dim_half], cos[..., rope_dim_half:]) and torch.equal(
                sin[..., :rope_dim_half], sin[..., rope_dim_half:]
            ):
                cos = cos[..., :rope_dim_half].repeat_interleave(2, dim=-1)
                sin = sin[..., :rope_dim_half].repeat_interleave(2, dim=-1)
        elif cache_dim == 2 * D:
            # Defensive handling for malformed caches produced by repeat(1,1,2)
            # in upstream interleaved cache recording.
            logger.warning_once(
                "[310P MLA] RoPE cache width=%s is 2x expected dim=%s; truncating and normalizing.",
                cache_dim,
                D,
            )
            cos = cos[..., :D]
            ...
            raise RuntimeError(
                f"Unexpected RoPE cos/sin width in _rope_single: got {cache_dim}, expected one of "
                f"{{{rope_dim_half}, {D}, {2 * D}}}"
            )

        cos = cos.reshape(B, 1, D).to(dtype=x.dtype)
        sin = sin.reshape(B, 1, D).to(dtype=x.dtype)
    ...
    def _forward_decode(
        ...
        k_nope = kv_cache[0].permute(0, 2, 1, 3).contiguous()
        k_pe = kv_cache[1].permute(0, 2, 1, 3).contiguous()

        if not hasattr(self, "_debug_dumped_cache_read"):
            self._debug_dumped_cache_read = True
            try:
                block_table = decode_meta["block_table"]
                first_block = int(block_table[0, 0].item()) if block_table.numel() > 0 else -1
                if 0 <= first_block < k_nope.shape[0]:
                    logger.info(
                        "[310P MLA][DEBUG] cache read k_nope shape=%s dtype=%s",
                        tuple(k_nope.shape),
                        k_nope.dtype,
                    )
                    logger.info(
                        "[310P MLA][DEBUG] cache read k_pe shape=%s dtype=%s",
                        tuple(k_pe.shape),
                        k_pe.dtype,
                    )
                    logger.info(
                        "[310P MLA][DEBUG] cache read block_table head: %s",
                        block_table[0, :10].tolist(),
                    )
                    k_nope_sample = k_nope[first_block, :2, :2, :4].detach().cpu()
                    k_pe_sample = k_pe[first_block, :2, :2, :4].detach().cpu()
                    logger.info(
                        "[310P MLA][DEBUG] cache read k_nope sample: %s",
                        k_nope_sample.tolist(),
                    )
                    logger.info(
                        "[310P MLA][DEBUG] cache read k_pe sample: %s",
                        k_pe_sample.tolist(),
                    )
                else:
                    logger.info("[310P MLA][DEBUG] cache read block invalid: %s", first_block)
            except Exception as e:
                logger.warning("[310P MLA][DEBUG] cache read debug dump failed: %s", e)
